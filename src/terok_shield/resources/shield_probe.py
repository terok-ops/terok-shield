...
Usage::
    ...
    python3 shield_probe.py HOST [PORT]
...
def probe(host: str, port: int = 443, timeout: float = 3.0) -> dict:
    ...
    try:
        ...
        for cmsg_level, cmsg_type, cmsg_data in ancdata:
            if cmsg_level == _SOL_IP and cmsg_type == _IP_RECVERR:
                if len(cmsg_data) < _SOCK_EE_SIZE:
                    continue
                ee_errno, ee_origin, ee_type, ee_code, ee_info, ee_data = struct.unpack(
                    _SOCK_EE_FMT, cmsg_data[:_SOCK_EE_SIZE]
                )
                if ee_origin != _SO_EE_ORIGIN_ICMP:
                    continue
                code_name = _ICMP_UNREACH_CODES.get(ee_code, f"code-{ee_code}")
                return {
                    **base,
                    "result": "icmp-error",
                    "icmp_type": ee_type,
